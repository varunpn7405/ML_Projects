    ...
    if bboxs:
        ...
        for bbox in bboxs:
            ...
            if score > confidence:
                ...
                blurLabel=f"Blur : {str(blurValue)}"
                Label=f"Score {int(score*100)}% {blurLabel}"
                yt,xt=cv2.getTextSize(blurLabel,cv2.FONT_HERSHEY_PLAIN,1,2)[0]

                cv2.rectangle(imgOut, (x, y), (xmax, ymax), clr, 1)
                text_size = cv2.getTextSize(Label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)[0]
                cv2.rectangle(imgOut, (x, y - text_size[1]), (x + text_size[0], y), clr, -1)
                cv2.putText(imgOut, Label, (x, y-2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
